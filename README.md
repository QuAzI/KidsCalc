...
Для Android значение `SupportedOSPlatformVersion=24.0` задаёт минимальную
версию устройства. Целевой API определяется `net10.0-android` отдельно и
соответствует Android 16 (API 36), поэтому снижение минимальной версии не
меняет соответствие требованиям Google Play к целевому API.
